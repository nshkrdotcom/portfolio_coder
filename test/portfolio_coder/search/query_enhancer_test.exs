defmodule PortfolioCoder.Search.QueryEnhancerTest do
  ...
  defmodule MockLLM do
    ...
    defp response_for(prompt) do
      cond do
        rewrite_prompt?(prompt) -> rewrite_response(prompt)
        expand_prompt?(prompt) -> expand_response(prompt)
        decompose_prompt?(prompt) -> decompose_response(prompt)
        code_rewrite_prompt?(prompt) -> "user login authentication handler"
        true -> "default response"
      end
    end

    defp rewrite_prompt?(prompt) do
      contains_any?(prompt, ["rewrite", "optimizer"])
    end

    defp expand_prompt?(prompt) do
      contains_any?(prompt, ["expand", "synonyms"])
    end

    defp decompose_prompt?(prompt) do
      contains_any?(prompt, ["decompose", "sub-questions"])
    end

    defp code_rewrite_prompt?(prompt) do
      String.contains?(prompt, "Transform") and String.contains?(prompt, "code search")
    end

    defp rewrite_response(prompt) do
      cond do
        String.contains?(prompt, "Hey, how does Phoenix") -> "how Phoenix LiveView works"
        String.contains?(prompt, "Can you help me") -> "find auth code"
        String.contains?(prompt, "hello") -> "greeting test"
        true -> "rewritten query"
      end
    end

    defp expand_response(prompt) do
      cond do
    ...
    defp decompose_response(prompt) do
      cond do
    ...
    defp contains_any?(prompt, values) do
      Enum.any?(values, &String.contains?(prompt, &1))
    end
  end
  ...
  describe "decompose/2" do
    test "breaks down comparison questions", %{opts: opts} do
      ...
      assert match?([_, _ | _], result.sub_questions)
